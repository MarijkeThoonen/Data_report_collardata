...
Dit is een verwijzing naar een figuur: @fig-ortho-{{label}}.
Het label van een figuur moet steeds beginnen met "fig-".
...
Dit is een verwijzing naar een tabel: @tbl-kuddes-{{label}}.
Het label van een tabel moet steeds beginnen met "tbl-".

```{r}
#| label: tbl-kuddes-{{label}}
#| eval: true
...
kable(herd, 
  ...
  tibble(a=1, b=2, c=3) %>%
    kable(caption = "tabel niet beschikbaar. Overzicht van de kuddes in de onderzochte graasblokken (diersoort, graasperiode, kuddegrootte, monitoring)")
}
...
```{r}
#| label: tbl-biotope-season-{{label}}
...
missing_seasons <- which(!(c("Winter", "Lente", "Zomer", "Herfst") %in%
                   colnames(biotope_counts)))
if (length(missing_seasons > 0)) {
  biotope_counts[c("Winter", "Lente", "Zomer", "Herfst")[missing_seasons]] <- NA
}
biotope_counts <- biotope_counts %>%
  mutate(
...
biotope_counts %>%
  ...
  tibble(a=1, b=2, c=3) %>%
    kable(caption = "Tabel niet beschikbaar. Collar positions per biotope per season/species")
}
```

```{r }
#| label: fig-biotope-bars-{{label}}
...
```{r}
#| label: fig-rel-avail-biotope-{{label}}
